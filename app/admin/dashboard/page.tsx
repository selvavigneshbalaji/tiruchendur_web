  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-muted-foreground">Manage the properties shown on the booking site.</p><h2 className="mt-1 text-xl font-semibold">Properties <span className="text-muted-foreground">({hotels.length})</span></h2></div><div className="flex flex-wrap gap-2"><Link href="/admin/dashboard/rooms" className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border px-2.5 text-sm font-medium hover:bg-muted"><Building2 /> Rooms & photos</Link><Button variant="outline" onClick={() => void loadHotels()} disabled={loading}><RefreshCw /> Refresh</Button><Button onClick={() => { setShowForm((current) => !current); setError("") }}><Plus /> Add property</Button></div></div>
